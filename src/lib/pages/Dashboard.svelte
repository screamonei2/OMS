<script lang="ts">
    ...
    type OrderStatus = 'Pendente' | 'Em Processamento' | 'Entregue' | 'Cancelado';
    ...
    async function fetchStats() {
        ...
        if (clientsData) {
            ...
            stats.activeClients = clientsData.filter(c => c.status === 'Ativo').length;
        }
        ...
        if (productsData) {
            ...
            stats.lowStockProducts = productsData.filter(p => p.stock < 10).length;
        }
        ...
        if (ordersData) {
            ...
            stats.totalRevenue = ordersData.reduce((sum, order) => sum + (order.total || 0), 0);
            ...
            stats.ordersToday = ordersData.filter(order => order.date === today).length;
    ...
    function updateCharts(ordersByDay: Array<{ date: string; count: number; revenue: number }>) {
    ...
    onMount(() => {
        ...
        return () => {
            ...
            [orderTrendsChart, statusChart, revenueChart].forEach(chart => chart?.destroy());
        };
    });
</script>
...
<div class="grid gap-4">
        ...
        <div class="card bg-base-100 card-border border-base-300">
            <div class="card-body">
                <div class="stat">
                    ...
                    <div class="stat-desc">{stats.activeClients} ativos</div>
                </div>
            </div>
        </div>
        ...
        <div class="card bg-base-100 card-border border-base-300">
            <div class="card-body">
                <div class="stat">
                    ...
                    <div class="stat-desc">{stats.lowStockProducts} com estoque baixo</div>
                </div>
            </div>
        </div>
        ...
        <div class="card bg-base-100 card-border border-base-300">
            <div class="card-body">
                <div class="stat">
                    ...
                    <div class="stat-value text-accent">{stats.ordersToday}</div>
        ...
        <div class="card bg-base-100 card-border border-base-300">
            <div class="card-body">
                <div class="stat">
                    ...
                    <div class="stat-value text-success">R$ {stats.totalRevenue.toFixed(2)}</div>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="card-body">
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div class="stat">
                    ...
                    <div class="stat-value text-warning">{stats.ordersByStatus.Pendente}</div>
                </div>
                <div class="stat">
                    <div class="stat-title">Em Processamento</div>
                    <div class="stat-value text-info">{stats.ordersByStatus['Em Processamento']}</div>
                </div>
                <div class="stat">
                    <div class="stat-title">Entregues</div>
                    <div class="stat-value text-success">{stats.ordersByStatus.Entregue}</div>
                </div>
                <div class="stat">
                    <div class="stat-title">Cancelados</div>
                    <div class="stat-value text-error">{stats.ordersByStatus.Cancelado}</div>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div class="card bg-base-100 card-border border-base-300 lg:col-span-2">
            <div class="card-body flex flex-col md:flex-row items-start justify-between gap-4">
                <div class="w-full">
