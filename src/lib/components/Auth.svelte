<script lang="ts">
    ...
    function validateForm(): boolean {
        if (!email || !password) {
            error = { message: 'Por favor, preencha todos os campos', status: 400 };
            return false;
        }
        ...
        if (password.length < 6) {
            error = { message: 'A senha deve ter pelo menos 6 caracteres', status: 400 };
            return false;
        }

        const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
        if (!emailRegex.test(email)) {
            error = { message: 'Por favor, insira um email válido', status: 400 };
            return false;
        }
...
<div class="min-h-screen flex items-center justify-center">
    <div class="card w-96 bg-base-100 shadow-xl">
        <div class="card-body">
            ...
            {#if error}
                <div class="alert {error.status === 200 ? 'alert-success' : 'alert-error'} mb-4">
